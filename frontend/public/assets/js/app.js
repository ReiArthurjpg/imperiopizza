    ...
    function empty(title, text) { return `<div class="flex flex-col items-center justify-center p-8 text-center text-gray-500 bg-gray-50/50 rounded-lg border border-gray-200 border-dashed"><strong class="block text-sm font-semibold text-gray-700 mb-1">${esc(title)}</strong><span class="text-xs">${esc(text)}</span></div>` }
    ...
    function rankHtml(op) { const r = ranking(op), total = stats(op).pizzas; if (!r.length) return empty('Sem resultado de montagem', 'O ranking aparece após o registro das comandas.'); return `<div class="w-full bg-white rounded-lg overflow-hidden"><table class="w-full text-left text-sm whitespace-nowrap"><thead class="bg-gray-50/80 border-b border-[#E7E7E7] text-[#737373] text-xs font-semibold"><tr><th class="px-4 py-3 w-16 text-center">Pos</th><th class="px-4 py-3">Montador</th><th class="px-4 py-3 text-right">Pizzas</th><th class="px-4 py-3 text-right">Comandas</th><th class="px-4 py-3 text-right">Erros</th></tr></thead><tbody class="divide-y divide-[#E7E7E7]">${r.map((x, i) => `<tr class="hover:bg-gray-50 transition-colors"><td class="px-4 py-3 text-center"><span class="inline-flex items-center justify-center w-6 h-6 rounded-full text-xs font-bold ${i === 0 ? 'bg-[#FFF9E5] text-[#D97706] ring-1 ring-[#FDE68A]' : i === 1 ? 'bg-gray-100 text-gray-600 ring-1 ring-gray-200' : i === 2 ? 'bg-[#FFF2E5] text-[#B85C00] ring-1 ring-[#FCDCB5]' : 'bg-transparent text-[#737373]'}">${i + 1}º</span></td><td class="px-4 py-3"><div class="flex flex-col"><strong class="text-[#171717] font-medium">${esc(x.name)}</strong><span class="text-[11px] text-[#737373] mt-0.5">${total ? ((x.pizzas / total) * 100).toFixed(1) : '0.0'}% das pizzas</span></div></td><td class="px-4 py-3 text-right font-semibold text-[#171717]">${x.pizzas}</td><td class="px-4 py-3 text-right text-[#737373]">${x.commands}</td><td class="px-4 py-3 text-right"><span class="inline-flex px-2 py-0.5 rounded text-xs font-medium ${x.errors > 0 ? 'bg-red-50 text-red-700' : 'text-[#737373]'}">${x.errors}</span></td></tr>`).join('')}</tbody></table></div>` }
...
if (!op || op.status === 'draft') { el.productionGate.innerHTML = gateCard("Operação não iniciada", "Cadastre a equipe do dia e inicie a operação para liberar a tela de produção.", "Ir para equipe", "team", "chef-hat"); el.productionContent.classList.add('hidden'); el.closeKitchenBtn.disabled = true; el.reopenKitchenBtn.classList.add('hidden'); if (el.manageTeamBtn) el.manageTeamBtn.classList.add('hidden'); el.closeKitchenBtn.classList.add('hidden'); return } if (op.status === 'completed') { el.productionGate.innerHTML = gateCard("Operação finalizada", "Consulte os relatórios do dia para ver o histórico e resultados.", "Abrir relatórios", "reports", "bar-chart-2"); el.productionContent.classList.add('hidden'); el.closeKitchenBtn.disabled = true; el.reopenKitchenBtn.classList.add('hidden'); if (el.manageTeamBtn) el.manageTeamBtn.classList.add('hidden'); el.closeKitchenBtn.classList.add('hidden'); return } el.productionGate.innerHTML = ''; el.productionContent.classList.remove('hidden'); if (el.manageTeamBtn) el.manageTeamBtn.classList.remove('hidden'); el.closeKitchenBtn.disabled = !kitchenOpen; el.closeKitchenBtn.classList.toggle('hidden', !kitchenOpen); el.reopenKitchenBtn.classList.toggle('hidden', op.status !== 'kitchen_closed'); el.openRegisterCommandBtn.disabled = !kitchenOpen; el.addCommandBtn.disabled = !kitchenOpen; el.assemblerId.disabled = !kitchenOpen; el.commandNumber.disabled = !kitchenOpen; el.pizzaQty.disabled = !kitchenOpen; el.commandNote.disabled = !kitchenOpen; el.initialOven.disabled = !kitchenOpen; renderProductionSub(op); renderAssemblerFilter(op); renderCommandSuggestions(op); renderProductionRecent(op); renderProductionTable(op); if (typeof renderSweetPendingPanel === 'function') renderSweetPendingPanel(); }
    ...
    async function renderDispatch() { 
      renderHeader(); 
      const op = currentOperation(); 
      el.manageTeamDispatchBtn.disabled = op?.status === 'completed'; 
      if (!op || op.status === 'draft' || op.status === 'production_open') { 
        el.dispatchGate.innerHTML = gateCard(op?.status === 'production_open' ? 'Despacho disponível durante a operação' : 'Operação não iniciada', op?.status === 'production_open' ? 'As comandas enviadas pela cozinha já aparecem abaixo.' : 'Inicie a operação para usar o despacho.', op?.status === 'production_open' ? 'Abrir produção' : 'Ir para equipe', op?.status === 'production_open' ? 'production' : 'team', op?.status === 'production_open' ? 'chef-hat' : 'users'); 
        if (!op || op.status === 'draft') { 
          el.dispatchContent.classList.add('hidden'); 
          el.finishDayBtn.disabled = true; 
          el.manageTeamDispatchBtn.classList.add('hidden'); 
          el.finishDayBtn.classList.add('hidden'); 
          return 
        } 
      } else el.dispatchGate.innerHTML = ''; 
      ...
      el.dispatchContent.classList.toggle('hidden', !op || op.status === 'draft'); 
      if (!op) return; 

      try {
        const q = norm(el.dispatchSearch.value), f = el.dispatchFilter.value;
        const [statsRes, queueRes] = await Promise.all([
          fetch(`/api/operations/${op.id}/dispatch/stats`),
          fetch(`/api/operations/${op.id}/dispatch/queue?search=${encodeURIComponent(q)}&status=${encodeURIComponent(f)}`)
        ]);
        
        const stats = await statsRes.json();
        const queueData = await queueRes.json();
        
        const kpis = stats.stats || { aguardando: 0, conferido: 0, entrega: 0, pendingToFinish: 0 };
        const totalRecebidas = kpis.aguardando + kpis.conferido + kpis.entrega;
        
        el.dispatchSubtotals.innerHTML = [['Recebidas', totalRecebidas], ['Aguardando', kpis.aguardando], ['Conferidas', kpis.conferido], ['Saíram Entrega', kpis.entrega], ['Faltam zerar', kpis.pendingToFinish]].map(([l, v]) => `<div class="subtotal-item"><small>${l}</small><strong>${v}</strong></div>`).join(''); 
        
        const cs = queueData.queue || [];
        // Map backend fields to frontend expected fields for dispatchCard
        const mappedCs = cs.map(c => ({
          ...
          dispatch: {
            ...
            ketchup: c.ketchup,
            mayonnaise: c.mayonnaise,
            note: c.dispatch_note
          },
          statusTimes: { despacho: c.despacho_time },
          assemblerName: c.assemblerName || c.assembler_name
        }));
        
        el.dispatchGrid.innerHTML = mappedCs.map(dispatchCard).join(''); 
        
        const mode = el.viewListDispatchBtn?.classList.contains('text-[#1F6FB2]') ? 'list' : 'grid';
        if (mode === 'list') {
            el.dispatchTableBody.innerHTML = mappedCs.map(dispatchListRow).join('');
        }
        
        el.dispatchEmpty.classList.toggle('hidden', mappedCs.length === 0); 
        el.finishDayBtn.disabled = op.status !== 'kitchen_closed' || kpis.pendingToFinish > 0; 
        el.finishDayBtn.textContent = op.status === 'completed' ? 'Dia finalizado' : kpis.pendingToFinish > 0 ? `Faltam ${kpis.pendingToFinish} pedidos` : 'Finalizar o dia';
      } catch(e) {
...
function dispatchCard(c) { const d = c.dispatch, cls = d.status === 'liberado' ? 'b-released' : d.status === 'conferido' ? 'b-dispatch' : 'b-oven', label = d.status === 'liberado' ? 'Liberado' : d.status === 'conferido' ? 'Conferido' : 'Aguardando'; return `<article class="dispatch-card ${d.status === 'liberado' ? 'released' : ''}" data-dispatch-card="${c.id}"><div class="dispatch-head"><div><h4>Comanda #${String(c.number).padStart(3, '0')}</h4><p>${c.pizzas} pizza${c.pizzas > 1 ? 's' : ''} · montador ${formatAssemblers(c)} · ${formatTime(c.statusTimes.despacho)}</p></div><span class="badge ${cls}">${label}</span></div><div class="dispatch-options"><label class="option"><input type="checkbox" data-d-field="beverage" ${d.beverage ? 'checked' : ''}> Tem bebida</label><label class="option"><input type="checkbox" data-d-field="change" ${d.change ? 'checked' : ''}> Precisa de troco</label><label class="option"><input type="checkbox" data-d-field="ketchup" ${d.ketchup ? 'checked' : ''}> Ketchup</label><label class="option"><input type="checkbox" data-d-field="mayonnaise" ${d.mayonnaise ? 'checked' : ''}> Maionese</label></div><div class="inline"><div class="field"><label>Troco</label><input data-d-field="changeAmount" type="text" value="${esc(d.changeAmount || '')}" placeholder="Ex.: para R$ 100" ${d.change ? '' : 'disabled'}></div><div class="field"><label>Observação</label><input data-d-field="note" type="text" value="${esc(d.note || '')}" placeholder="Opcional"></div></div><div class="actions end mobile-stack"><button class="btn btn-soft btn-small" data-d-action="save" data-id="${c.id}">Salvar</button><button class="btn btn-orange btn-small" data-d-action="check" data-id="${c.id}">Conferido</button><button class="btn btn-green btn-small" data-d-action="release" data-id="${c.id}">Liberar pedido</button></div></article>` }
    ...
    function renderReports() { renderHeader(); const ops = [...state.operations].sort((a, b) => b.date.localeCompare(a.date)); if (!ops.length) { el.historyList.innerHTML = empty('Nenhuma operação', 'Crie a equipe do primeiro dia.'); el.reportOverview.innerHTML = empty('Sem dados', 'Os relatórios aparecerão aqui.'); el.reportCards.innerHTML = ''; return } if (!selectedReportOperationId || !getOperation(selectedReportOperationId)) selectedReportOperationId = ops[0].id; el.historyList.innerHTML = ops.map(o => { const s = stats(o); const isSelected = o.id === selectedReportOperationId; const baseClasses = "flex flex-col gap-2 p-3 rounded-lg border cursor-pointer transition-all duration-150"; const selectedClasses = isSelected ? "bg-blue-50/50 border-[#1F6FB2] shadow-[0_2px_8px_rgba(31,111,178,0.15)] ring-1 ring-[#1F6FB2]" : "bg-white border-[#E7E7E7] hover:border-gray-300 hover:bg-gray-50"; return `<div class="${baseClasses} ${selectedClasses}" data-report-op="${o.id}"><div class="flex items-center justify-between"><h4 class="text-sm font-semibold text-[#171717]">${formatDate(o.date)}</h4><span class="text-xs font-medium ${isSelected ? 'text-[#1F6FB2]' : 'text-[#737373]'}">${phaseLabel(o)}</span></div><p class="text-xs text-[#737373] flex items-center gap-1"><i data-lucide="users" class="w-3.5 h-3.5"></i> ${o.team.length} pessoas</p><div class="flex flex-wrap gap-1.5 mt-1"><span class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-medium bg-gray-100 text-gray-700 border border-gray-200">${fmt(s.pizzas)} pizzas</span><span class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-medium ${s.errors > 0 ? 'bg-red-50 text-red-700 border border-red-200' : 'bg-gray-100 text-gray-700 border border-gray-200'}">${s.errors} erros</span><span class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-medium bg-green-50 text-green-700 border border-green-200">${s.released} liberadas</span></div></div>` }).join(''); lucide.createIcons(); renderReportDetail(getOperation(selectedReportOperationId)) }
    function renderReportDetail(op) { 
      const s = stats(op), errRate = s.pizzas ? ((s.errors / s.pizzas) * 100).toFixed(1) : '0.0', mass = ensureMass(op), used = massConsumed(op); 
      el.reportOverview.innerHTML = `<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6 border-b border-[#E7E7E7] pb-4"><div><h3 class="text-xl font-bold text-[#171717]">${formatDate(op.date)}</h3><p class="text-sm text-[#737373] mt-1 flex items-center gap-2 flex-wrap"><span class="inline-flex items-center px-2 py-0.5 rounded-md text-xs font-medium bg-gray-100 text-gray-700">${phaseLabel(op)}</span><span>&bull; Início ${formatTime(op.startedAt)}</span><span>&bull; Cozinha ${formatTime(op.kitchenClosedAt)}</span><span>&bull; Final ${formatTime(op.completedAt)}</span></p></div></div><div class="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3 lg:gap-4 mb-8"><div class="bg-gray-50/50 p-3 rounded-lg border border-[#E7E7E7]"><span class="block text-[10px] font-medium text-[#737373] mb-1">Comandas</span><strong class="block text-lg font-bold text-[#171717]">${s.commands}</strong></div><div class="bg-gray-50/50 p-3 rounded-lg border border-[#E7E7E7]"><span class="block text-[10px] font-medium text-[#737373] mb-1">Pizzas equiv.</span><strong class="block text-lg font-bold text-[#171717]">${fmt(s.pizzas)}</strong></div><div class="bg-gray-50/50 p-3 rounded-lg border border-[#E7E7E7]"><span class="block text-[10px] font-medium text-[#737373] mb-1">Doces</span><strong class="block text-lg font-bold text-[#171717]">${fmt(s.sweet)}</strong></div><div class="bg-gray-50/50 p-3 rounded-lg border border-[#E7E7E7]"><span class="block text-[10px] font-medium text-[#737373] mb-1">Vulcão</span><strong class="block text-lg font-bold text-[#171717]">${fmt(s.volcano)}</strong></div><div class="bg-gray-50/50 p-3 rounded-lg border border-[#E7E7E7]"><span class="block text-[10px] font-medium text-[#737373] mb-1">Esfirras</span><strong class="block text-lg font-bold text-[#171717]">${fmt(s.esfihas, 0)}</strong></div><div class="${s.errors > 0 ? 'bg-red-50/50 border-red-100' : 'bg-gray-50/50 border-[#E7E7E7]'} p-3 rounded-lg border"><span class="block text-[10px] font-medium ${s.errors > 0 ? 'text-red-600' : 'text-[#737373]'} mb-1">Erros</span><strong class="block text-lg font-bold ${s.errors > 0 ? 'text-red-600' : 'text-[#171717]'}">${s.errors}</strong></div><div class="bg-gray-50/50 p-3 rounded-lg border border-[#E7E7E7]"><span class="block text-[10px] font-medium text-[#737373] mb-1">Batidas</span><strong class="block text-lg font-bold text-[#171717]">${mass.batches.length}</strong></div><div class="bg-gray-50/50 p-3 rounded-lg border border-[#E7E7E7]"><span class="block text-[10px] font-medium text-[#737373] mb-1">Farinha kg</span><strong class="block text-lg font-bold text-[#171717]">${fmt(used.flourKg)}</strong></div></div><div><h3 class="text-base font-semibold text-[#171717] mb-3">Ranking da montagem</h3><div class="bg-white rounded-lg border border-[#E7E7E7] overflow-hidden">${rankHtml(op)}</div></div>`; const attendanceReady = op.team.length > 0, productionReady = ['kitchen_closed', 'completed'].includes(op.status); el.reportCards.innerHTML = `<div class="bg-white rounded-xl border border-[#E7E7E7] shadow-[0_4px_12px_rgba(0,0,0,0.02)] flex flex-col h-full"><div class="p-4 border-b border-[#E7E7E7] bg-gray-50/30 grow"><div class="flex items-center gap-2 mb-2"><div class="p-1.5 bg-blue-50 text-[#1F6FB2] rounded-md"><i data-lucide="users" class="w-4 h-4"></i></div><h4 class="text-sm font-semibold text-[#171717]">Lista de presença</h4></div><p class="text-xs text-[#737373] leading-relaxed">Relação da equipe por setor, com campo de assinatura. Disponível após salvar a equipe.</p></div><div class="p-3 bg-white flex flex-col sm:flex-row gap-2 mt-auto"><button class="flex-1 px-3 py-2 text-xs font-semibold bg-white border border-[#E7E7E7] text-[#4B5563] rounded-lg hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors shadow-sm flex items-center justify-center gap-1.5" data-report-action="download-attendance" ${attendanceReady ? '' : 'disabled'}><i data-lucide="download" class="w-3.5 h-3.5"></i> Baixar relatório</button><button class="flex-1 px-3 py-2 text-xs font-semibold bg-[#1F6FB2] text-white rounded-lg hover:bg-[#1a5e98] disabled:opacity-50 disabled:cursor-not-allowed transition-colors shadow-sm flex items-center justify-center gap-1.5" data-report-action="print-attendance" ${attendanceReady ? '' : 'disabled'}><i data-lucide="printer" class="w-3.5 h-3.5"></i> Imprimir PDF</button></div></div><div class="bg-white rounded-xl border border-[#E7E7E7] shadow-[0_4px_12px_rgba(0,0,0,0.02)] flex flex-col h-full"><div class="p-4 border-b border-[#E7E7E7] bg-gray-50/30 grow"><div class="flex items-center gap-2 mb-2"><div class="p-1.5 bg-green-50 text-green-600 rounded-md"><i data-lucide="bar-chart-2" class="w-4 h-4"></i></div><h4 class="text-sm font-semibold text-[#171717]">Resultado da montagem</h4></div><p class="text-xs text-[#737373] leading-relaxed">Ranking por pizzas equivalentes, comandas, participação, especiais e erros. Disponível após encerrar a cozinha.</p></div><div class="p-3 bg-white flex flex-col sm:flex-row gap-2 mt-auto"><button class="flex-1 px-3 py-2 text-xs font-semibold bg-white border border-[#E7E7E7] text-[#4B5563] rounded-lg hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors shadow-sm flex items-center justify-center gap-1.5" data-report-action="download-production" ${productionReady ? '' : 'disabled'}><i data-lucide="download" class="w-3.5 h-3.5"></i> Baixar relatório</button><button class="flex-1 px-3 py-2 text-xs font-semibold bg-[#1F6FB2] text-white rounded-lg hover:bg-[#1a5e98] disabled:opacity-50 disabled:cursor-not-allowed transition-colors shadow-sm flex items-center justify-center gap-1.5" data-report-action="print-production" ${productionReady ? '' : 'disabled'}><i data-lucide="printer" class="w-3.5 h-3.5"></i> Imprimir PDF</button></div></div>`; lucide.createIcons(); }
...
<\/body><\/html>`}
    ...
    function productionReport(op) { const r = ranking(op), s = stats(op), rows = r.map((x, i) => `<tr><td class="rank">${i + 1}º</td><td>${esc(x.name)}</td><td>${fmt(x.pizzas)}</td><td>${x.commands}</td><td>${s.pizzas ? ((x.pizzas / s.pizzas) * 100).toFixed(1) : '0.0'}%</td><td>${x.errors}</td></tr>`).join(''); const errors = op.commands.filter(c => c.error?.active).map(c => `<tr><td>#${String(c.number).padStart(3, '0')}</td><td>${formatAssemblers(c)}</td><td>${esc(c.error.type)}</td><td>${esc(c.error.note || '—')}</td></tr>`).join(''); return reportShell('Resultado da Montagem', op, `<p><strong>Total:</strong> ${s.commands} comandas · ${fmt(s.pizzas)} pizzas equivalentes · ${fmt(s.physicalPizzas)} pizzas físicas · ${fmt(s.volcano)} vulcão · ${fmt(s.sweet)} doces · ${fmt(s.esfihas, 0)} esfirras · ${s.errors} ocorrências.</p><table><thead><tr><th>Rank</th><th>Montador</th><th>Pizzas equivalentes</th><th>Comandas</th><th>Participação</th><th>Erros</th></tr></thead><tbody>${rows}</tbody></table><h2 style="margin-top:22px">Ocorrências</h2>${errors ? `<table><thead><tr><th>Comanda</th><th>Montador</th><th>Tipo</th><th>Descrição</th></tr></thead><tbody>${errors}</tbody></table>` : '<p>Nenhuma ocorrência registrada.</p>'}`) }
    ...
    // despacho
    [el.dispatchSearch, el.dispatchFilter].forEach(x => x.addEventListener(x.tagName === 'INPUT' ? 'input' : 'change', renderDispatch)); el.clearDispatchFilters.addEventListener('click', () => { el.dispatchSearch.value = ''; el.dispatchFilter.value = ''; renderDispatch() }); el.dispatchGrid.addEventListener('change', e => { if (e.target.dataset.dField === 'change') { const card = e.target.closest('[data-dispatch-card]'), amt = card.querySelector('[data-d-field="changeAmount"]'); amt.disabled = !e.target.checked; if (!e.target.checked) amt.value = '' } }); 
el.dispatchGrid.addEventListener('click', async e => { 
  const b = e.target.closest('[data-d-action]'); 
  if (!b) return; 
  const op = currentOperation(); 
  if (!op) return;
  const card = e.target.closest('[data-dispatch-card]'); 
  if (!card) return; 
  const cmdId = b.dataset.id;
  
  b.disabled = true;
  const originalText = b.innerHTML;
  b.innerHTML = '<i class="lucide-loader w-4 h-4 animate-spin"></i>';
  
  try {
    if (b.dataset.dAction === 'check') { 
      await fetch(`/api/operations/${op.id}/commands/${cmdId}/dispatch/check`, { method: 'POST' });
      toast('Conferência salva.');
    } else if (b.dataset.dAction === 'release') { 
      await fetch(`/api/operations/${op.id}/commands/${cmdId}/dispatch/send-delivery`, { method: 'POST' });
      toast('Pedido saiu para entrega.');
    } else if (b.dataset.dAction === 'revert') {
      await fetch(`/api/operations/${op.id}/commands/${cmdId}/dispatch/revert`, { method: 'POST' });
      toast('Pedido revertido para aguardando.');
    } else if (b.dataset.dAction === 'save') {
      const get = n => { const x = card.querySelector(`[data-d-field="${n}"]`); return x?.type === 'checkbox' ? (x.checked ? 1 : 0) : (x?.value || '').trim() }; 
      const data = {
          beverage: get('beverage'),
          change: get('change'),
          changeAmount: get('change') ? get('changeAmount') : '',
          ketchup: get('ketchup'),
          mayonnaise: get('mayonnaise'),
          note: get('note')
      };
      await fetch(`/api/operations/${op.id}/commands/${cmdId}/dispatch`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
...
if (el.finishDayBtn) el.finishDayBtn.addEventListener('click', async () => {
    const op = currentOperation();
    if (!op || op.status !== 'kitchen_closed') return toast('Encerre a cozinha antes de finalizar o dia.', 'warn');
    
    // We already fetch pendingToFinish dynamically, but we can rely on the button disabled state.
    if (el.finishDayBtn.disabled) return;
    
    if (confirm('Tem certeza que deseja finalizar a operação de hoje? Nenhuma comanda poderá ser editada depois.')) {
        try {
            el.finishDayBtn.disabled = true;
            el.finishDayBtn.textContent = 'Finalizando...';
            await fetch(`/api/operations/${op.id}/finish-day`, { method: 'POST' });
            toast('Operação finalizada!');
            window.location.reload();
        } catch(e) {
            toast('Erro ao finalizar', 'error');
            el.finishDayBtn.disabled = false;
            el.finishDayBtn.textContent = 'Finalizar o dia';
        }
    }
});
    ...
    rankHtml = function (op) {
      ...
      return `<div class="w-full bg-white rounded-lg overflow-hidden"><table class="w-full text-left text-sm whitespace-nowrap"><thead class="bg-gray-50/80 border-b border-[#E7E7E7] text-[#737373] text-xs font-semibold"><tr><th class="px-4 py-3 w-16 text-center">Pos</th><th class="px-4 py-3">Montador</th><th class="px-4 py-3 text-right">Pizzas (Equiv.)</th><th class="px-4 py-3 text-right">Comandas</th><th class="px-4 py-3 text-right">Erros</th></tr></thead><tbody class="divide-y divide-[#E7E7E7]">${r.map((x, i) => `<tr class="hover:bg-gray-50 transition-colors"><td class="px-4 py-3 text-center"><span class="inline-flex items-center justify-center w-6 h-6 rounded-full text-xs font-bold ${i === 0 ? 'bg-[#FFF9E5] text-[#D97706] ring-1 ring-[#FDE68A]' : i === 1 ? 'bg-gray-100 text-gray-600 ring-1 ring-gray-200' : i === 2 ? 'bg-[#FFF2E5] text-[#B85C00] ring-1 ring-[#FCDCB5]' : 'bg-transparent text-[#737373]'}">${i + 1}º</span></td><td class="px-4 py-3"><div class="flex flex-col"><strong class="text-[#171717] font-medium">${esc(x.name)}</strong><span class="text-[11px] text-[#737373] mt-0.5">${total ? ((x.pizzas / total) * 100).toFixed(1) : '0.0'}% da produção equivalente</span></div></td><td class="px-4 py-3 text-right font-semibold text-[#171717]">${fmt(x.pizzas)}</td><td class="px-4 py-3 text-right text-[#737373]">${x.commands}</td><td class="px-4 py-3 text-right"><span class="inline-flex px-2 py-0.5 rounded text-xs font-medium ${x.errors > 0 ? 'bg-red-50 text-red-700' : 'text-[#737373]'}">${x.errors}</span></td></tr>`).join('')}</tbody></table></div>`;
    };
...
async function openDispatchIntake(commandId = '') {
      const op = currentOperation(); 
      if (!op || ['draft', 'completed'].includes(op.status)) return toast('O atendimento não está disponível.', 'warn');
      el.dispatchModalTitle.textContent = 'Puxar comanda do forno'; 
      el.dispatchModalSubtitle.textContent = 'Selecione uma comanda que já entrou no forno.';
      el.ovenCommandPickerBlock.classList.remove('hidden'); 
      el.dispatchCommandId.value = commandId || ''; 
      el.ovenCommandSearch.value = '';
      resetDispatchFields(); 
      
      el.ovenCommandList.innerHTML = '<div class="p-4 text-center text-gray-500">Carregando...</div>';
      ...
      try {
        ...
        renderOvenPicker(commandId);
      } catch(e) {
        console.error(e);
        toast('Erro ao carregar forno', 'error');
      }
      
      const c = commandId ? window._ovenList?.find(x => x.id === commandId) : null;
      el.dispatchSelectedSummary.innerHTML = c ? selectedDispatchHtml(c) : ''; 
      el.dispatchSelectedSummary.classList.toggle('show', !!c);
      el.receiveDispatchBtn.classList.remove('hidden'); 
      el.checkDispatchBtn.classList.add('hidden'); 
      el.outForDeliveryBtn.classList.add('hidden');
    }
...
el.receiveDispatchBtn.addEventListener('click', async () => {
    const op = currentOperation(), cmdId = el.dispatchCommandId.value;
    if (!op || !cmdId) return toast('Selecione uma comanda.', 'warn');
    
    el.receiveDispatchBtn.disabled = true;
    el.receiveDispatchBtn.textContent = 'Puxando...';
    try {
        await fetch(`/api/operations/${op.id}/dispatch/pull-from-oven`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        // Also save data
        const get = (id) => $(id).type === 'checkbox' ? ($(id).checked ? 1 : 0) : $(id).value.trim();
        const data = {
          beverage: get('dispatchBeverage'),
          change: get('dispatchChange'),
          changeAmount: get('dispatchChange') ? get('dispatchChangeAmount') : '',
          ketchup: get('dispatchKetchup'),
          mayonnaise: get('dispatchMayonnaise'),
          note: get('dispatchNote')
        };
        await fetch(`/api/operations/${op.id}/commands/${cmdId}/dispatch`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
        ...
        toast('Comanda recebida no atendimento.');
        el.dispatchCommandModal.classList.remove('show');
        await renderDispatch();
    } catch(e) {
        console.error(e);
        toast('Erro ao puxar do forno.', 'error');
    } finally {
        el.receiveDispatchBtn.disabled = false;
        el.receiveDispatchBtn.textContent = 'Puxar para atendimento';
    }
});

el.checkDispatchBtn.addEventListener('click', async () => {
    const op = currentOperation(), cmdId = el.dispatchCommandId.value;
    if (!op || !cmdId) return;
    el.checkDispatchBtn.disabled = true;
    try {
        await fetch(`/api/operations/${op.id}/commands/${cmdId}/dispatch/check`, { method: 'POST' });
        
        const get = (id) => $(id).type === 'checkbox' ? ($(id).checked ? 1 : 0) : $(id).value.trim();
        const data = { beverage: get('dispatchBeverage'), change: get('dispatchChange'), changeAmount: get('dispatchChange') ? get('dispatchChangeAmount') : '', ketchup: get('dispatchKetchup'), mayonnaise: get('dispatchMayonnaise'), note: get('dispatchNote') };
        await fetch(`/api/operations/${op.id}/commands/${cmdId}/dispatch`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) });
        
        toast('Conferência salva.');
        el.dispatchCommandModal.classList.remove('show');
        await renderDispatch();
    } catch(e) {
        toast('Erro ao conferir', 'error');
    } finally {
        el.checkDispatchBtn.disabled = false;
    }
});
